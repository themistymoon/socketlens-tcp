...
| 202  | `TEST ACCEPTED`   | สงวนไว้สำหรับการรันแบบอะซิงโครนัสในอนาคต เวอร์ชัน 0.1 ไม่มีเส้นทางใดตอบรหัสนี้ เพราะ `RUN_TEST` รันแบบซิงโครนัสเสมอ |
...
| `fragmentation` | ข้อความหนึ่งข้อความจากการเขียนหกครั้ง (คนละสถานการณ์กับตัวอย่างที่ 5 ซึ่งเขียนเจ็ดครั้ง) |
...
ได้ตรวจสอบขั้นตอนจริงเมื่อวันที่ 29 สิงหาคม 2569 ด้วย Wireshark 4.6.7 และ Npcap 1.88
บน commit `4d1c6a1` หลักฐานอยู่ใน `evidence/wireshark-2026-08-29/` ประกอบด้วยไฟล์จับแพ็กเก็ต
สองไฟล์และภาพหน้าจอ 11 ภาพ ผลที่เห็นตรงกับสถานการณ์สำคัญ ได้แก่ข้อความ `utf8` 12 อักขระ
ใช้ 36 ไบต์ ข้อความ JSON มี `Content-Length: 47` การส่งแบบ `fragmentation` ปรากฏเป็น
เซกเมนต์ที่มีเพย์โหลดหกเซกเมนต์ในการรันครั้งนี้ การส่งแบบ `coalescing` มีเพย์โหลด 247 ไบต์
หนึ่งเซกเมนต์แล้วได้รับคำตอบสองข้อความ และคำขอ `malformed` ได้รับ `400 BAD REQUEST`
ก่อนเซิร์ฟเวอร์ส่ง FIN ทั้งนี้จำนวนเซกเมนต์หกเซกเมนต์ไม่ได้พิสูจน์ว่าการเขียนของแอปพลิเคชัน
ต้องเป็นหกครั้งเสมอ เพราะขอบเขตทั้งสองชนิดไม่เท่ากันโดยทั่วไป
...
ผู้ชมสามารถเห็นข้อความหนึ่งข้อความถูกส่งด้วยการเขียนเจ็ดครั้ง (ตัวอย่างที่ 5) และเห็นข้อความสองข้อความ
ถูกส่งด้วยการเขียนครั้งเดียว (ตัวอย่างที่ 6) แล้วเห็นว่าปลายทางยังคงตีความได้ถูกต้องทั้งสองกรณี
